...
async def help(ctx):
	...
	embed=discord.Embed(title=":bulb: Help :bulb:", description="List of Commands", clolor=discord.Color.dark_green)
	embed.set_thumbnail(url="https://i.imgur.com/Xq4QOU8.png")
	embed.add_field(name="Moderation/User Info", value=".kick, .ban, .user")
	embed.add_field(name="Fun", value=".owncommand, .hello, .astrology")
	embed.add_field(name="Neccessary", value=".start")
	embed.add_field(name="Economy", value=".bitcoin")
	embed.add_field(name="Mathematics", value=".square")
	embed.add_field(name="Personal", value=".botinfo, .gitrepo, .owneronfo")
	embed.set_footer(name="These are the full list of Commands")
	await message.channel.send(content=None, embed=embed)
